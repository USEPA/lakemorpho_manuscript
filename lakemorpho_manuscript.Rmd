...
In R, there are many ways to iterate. For simplicity and clarity we use a `for()` loop to iterate through all lakes and calculate the full suite of lake metrics with `calcLakeMetrics()`.  We will utilize the `elevatr` package to access elevation data [@hollister_elevatr]. <!-- Funny business with elevation data and projections.  This assumes z units are same as CRS!  Not the case here as CRS if feet and elevation is m! Change to elevatr... --> 

```{r calc_all, cache=TRUE}
library(elevatr)
output <- data.frame()
for(i in seq_along(ri_lakes)){
  dem <- get_elev_raster(ri_lakes[i,],z = 12, expand = 1000)
  lmorph <- lakeSurroundTopo(inLake = ri_lakes[i,], inElev = dem)
  lmetric <- calcLakeMetrics(lmorph, bearing = 270, pointDens = 100)
  output <- rbind(output,
                  data.frame(NAME = ri_lakes[i,]$NAME, data.frame(lmetric)))
}
```
